$(function (){
      ...
      down : {
        ...
        callback : function (){ //必选，刷新函数，根据具体业务来编写，比如通过ajax从服务器获取新数据；
          ...
          render(function (info){
            ...
            //需要手动在数据回来后, 结束下拉刷新 (调用 mui('.mui-scroll-wrapper').pullRefresh() 原型上的方法)
  ...
  //disablePullupToRefresh()  禁用上拉加载
  //enablePullupToRefresh()   启用上拉加载
  //endPullupToRefresh()      结束上拉加载
  //endPulldownToRefresh()    结束下拉刷新
  //pulldownLoading()         正在加载下拉刷新
  //pullupLoading()           正在加载下拉加载


  //3. 点击价格或者库存, 切换 current, 实现排序
  //绑定点击事件
  //mui 框架中 a 标签的点击事件click 不起作用, 这里使用 tap 事件
  $('.lt_sort a[data-type]').on("tap",function (){
    ...
    //调用 mui('.mui-scroll-wrapper').pullRefresh() 原型上的方法, 下拉刷新
    mui('.mui-scroll-wrapper').pullRefresh().pulldownLoading();
